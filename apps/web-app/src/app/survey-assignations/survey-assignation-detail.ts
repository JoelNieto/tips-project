...
@Component({
  ...
  template: `
    <div class="space-y-6">
      ...
      >
        Emails are not sent automatically yet — copy the invite links below to
        share manually.
      </div>
      ...
      @if (loading()) {
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-5">
          ...
          @if (
            completedCount() === a.invitees.length && a.invitees.length > 0
          ) {
            <p class="mt-2 text-xs text-emerald-600 font-medium">
              All invitees have completed the survey
            </p>
          }
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
              ...
              <tbody class="divide-y divide-slate-200 bg-white">
                @for (inv of a.invitees; track inv.id) {
                  <tr>
                    <td
                      class="whitespace-nowrap px-6 py-4 text-sm text-slate-900"
                    >
                      {{ inv.email }}
                    </td>
                    <td
                    ...
                    <td class="whitespace-nowrap px-6 py-4">
                      @if (inv.employeeId) {
                        <span class="inline-flex flex-col gap-0.5">
                          ...
                          @if (inv.employee?.position; as position) {
          ...
          @if (activeTab() === 'results') {
            @if (resultsLoading()) {
              ...
              <div class="p-8 text-center">
                <span class="material-symbols-outlined text-4xl text-red-300"
                  >error</span
                >
                <p class="mt-4 text-red-700 font-medium">
      ...
      .subscribe({
        next: (result) => {
          ...
          if (result.data?.surveyAssignation) {
            this.assignation.set(
              result.data.surveyAssignation as AssignationDetail,
            );
            this.error.set(null);
          }
          if (result.error) {
